...
rtclient.Authorizer.prototype.authorize = function(onAuthComplete) {
  ...
  var handleAuthResult = function(authResult) {
    if (authResult && !authResult.error) {
      ...
      _this.fetchUserId(function(){onAuthComplete(true)});
    } else {
      _this.authButton.disabled = false;
      _this.authButton.onclick = authorizeWithPopup;
      _this.fetchUserId(function(){onAuthComplete(false)});
    }
...
rtclient.getMetadata = function(fileId) {
  gapi.client.load('drive', 'v2', function() {
    gapi.client.drive.files.get({
      'fileId' : fileId
    }).execute(rtclient.onMetadata);
  });
}
...
rtclient.RealtimeLoader = function(options) {
  ...
  this.onAuth = rtclient.getOption(options, 'onAuth');
  this.onFileLoaded = rtclient.getOption(options, 'onFileLoaded');
  this.onMetadata = rtclient.getOption(options, 'onMetadata', function(){});
 ...
 */
rtclient.RealtimeLoader.prototype.start = function( success ) {
  // Bind to local context to make them suitable for callbacks.
  var _this = this;
  this.authorizer.start(function( success ) {
    if (_this.registerTypes) {
      _this.registerTypes();
    }
    if (_this.onAuth) {
      _this.onAuth( success );
    }
    _this.load();
  });
  //_this.load();
